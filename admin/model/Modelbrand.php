<?php
    ...
    class ModelBrand{
       ...
       public function addBrand($conn){
           $stmt = $conn->prepare("INSERT INTO brand(brand_title) VALUES (:brand_title)");
           $stmt->execute(array(
       ...
       public static function deleteBrand($conn,$brand_title){
            $stmt = $conn->prepare("DELETE FROM brand WHERE brand_title=:brand_title");
            $stmt->execute(array(
       ...
       public static function updatebrand($conn,$brand_title,$brand_id){
           $stmt = $conn->prepare("UPDATE brand SET brand_title=:brand_title where brand_id = :brand_id");
           $stmt->execute(array(
               'brand_title'=> $brand_title,
               'brand_id' => $brand_id
           ));
